...
var {Config} = require('./cijoe/config');
var {Build} = require('./cijoe/build');
...
Object.defineProperties(CiJoe.prototype, {
    ...
    build: {
        value: function () {
            if (this.isBuilding()) {
                if (this.buildAllFile) {
                    fs.touch(this.buildAllFile);
                }
                return false;
            }
    ...
    doBuild: {
        value: function () {
            try {
                ...
                status === 0 ? this.buildWorked(output.content) :
                        this.buildFailed('', output.content);
            } catch (error) {
                log.error('Exception building:', error);
                this.buildFailed('', error);
            }
        }
    },
    runnerCommand: {
        value: function () {
            var runner;
            try {
                runner = new Config('cijoe.runner', this.projectPath)
                        .toString();
            } catch (error) {
                runner = '';
            }
    ...
    gitSha: {
        value: function () {
            return command('/usr/bin/git rev-parse origin/' + this.gitBranch(), {
                dir: this.projectPath
            });
        }
    },
    gitUpdate: {
        value: function () {
            command('/usr/bin/git fetch origin', {dir: this.projectPath});
            command('/usr/bin/git reset --hard origin/' + this.gitBranch(), {
                dir: this.projectPath
            });
    ...
    gitBranch: {
        value: function () {
            var branch;
            try {
                branch = new Config('cijoe.branch', this.projectPath)
                        .toString();
            } catch (error) {
                branch = '';
            }
